...
const Header = ({ input, setInput }) => {
  ...
  return (
    <header className="shadow-2xl py-4 mb-4 flex items-center justify-around sticky top-0 bg-white">
      ...
      <div className="flex gap-8 items-center w-2/6 justify-between">
        <div className="w-full">
          <label className="relative block ">
            ...
            <input className="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Search for product..." type="text" name="search"
        ...
        <div className="flex gap-3 items-center">
          {
            userInfo?.userAvailable || userInfo.adminAvailable ?
              <div className="relative inline-block text-left" onClick={() => setDropdown(!dropdown)}>
                <div>
                  <button type="button" className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true">
                    {userInfo?.userAvailable?.name || userInfo?.adminAvailable?.type}
                  ...
                  <div className="absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
                    ...
                    {userInfo?.userAvailable ?
                      <div className="py-1" role="none">
                        <NavLink to="/orders" className="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabIndex="-1" id="menu-item-0">Orders</NavLink>
                      </div>
                      :
                      <div className="py-1" role="none">
                        <NavLink to="/create-product" className="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabIndex="-1" id="menu-item-0">Create Product</NavLink>
                        <NavLink to="/users" className="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabIndex="-1" id="menu-item-0">All Users</NavLink>
                      </div>
                    }

                    <div className="py-1" role="none">
                      <NavLink to="#" className="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabIndex="-1" id="menu-item-6"
                        onClick={() => {
                          localStorage.clear()
